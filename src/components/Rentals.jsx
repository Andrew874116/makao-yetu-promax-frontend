...
export default function Rentals() {
  ...
  useEffect(() => {
    fetch(`${API}/api/get_properties?type=rent`)
      .then(res => res.json())
      .then(data => { setProperties(data); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  if (loading) return <div style={{ background: '#0D0D2B', minHeight: '100vh', color: 'white', textAlign: 'center', padding: '4rem' }}>Loading...</div>;
  ...
  return (
    <div style={{ background: '#0D0D2B', minHeight: '100vh', padding: '2rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: '2.5rem', color: '#F5D200', marginBottom: '2rem' }}>🏠 Properties for Rent</h1>
          ...
          {properties.map(prop => (
            <Link key={prop.id} to={`/property/${prop.id}`} style={{ background: '#1A1A3E', borderRadius: '16px', overflow: 'hidden', textDecoration: 'none', color: 'white', border: '1px solid #2E2E5E' }}>
              ...
              <div style={{ padding: '1rem' }}>
                <h3>{prop.title}</h3>
                <p style={{ color: '#F5D200', fontSize: '1.3rem', fontWeight: 'bold' }}>Ksh {Number(prop.price).toLocaleString()}/mo</p>
                <p>📍 {prop.location}</p>
              </div>
            </Link>
          ))}
