...
export default function Signin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('is_verified', data.is_verified || 0);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0D0D2B', padding: '1rem' }}>
      <div style={{ background: '#1A1A3E', padding: '2rem', borderRadius: '20px', width: '100%', maxWidth: '400px', border: '1px solid #2E2E5E' }}>
        <h1 style={{ textAlign: 'center', color: '#F5D200', marginBottom: '1rem' }}>Makao Yetu</h1>
        <p style={{ textAlign: 'center', color: 'rgba(255,255,255,.6)', marginBottom: '2rem' }}>Sign in to your account</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '0.75rem', background: '#E63030', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>{loading ? 'Signing in...' : 'Sign In'}</button>
        </form>
        <p style={{ textAlign: 'center', marginTop: '1rem' }}><Link to="/forgot-password" style={{ color: '#F5D200' }}>Forgot password?</Link></p>
        <p style={{ textAlign: 'center', marginTop: '1rem' }}>Don't have an account? <Link to="/signup" style={{ color: '#F5D200' }}>Sign Up</Link></p>
      </div>
    </div>
  );
}